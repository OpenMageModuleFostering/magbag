...
class Mage_Checkout_CartController extends Mage_Core_Controller_Front_Action
{
    ...
    public function addAction()
    {
        ...
        try {
			...
			print_r($this->_getCart()->getQuote()->getItemsCollection());
			exit;
			
			//print_r($this->_getSession()->getQuote()->getData());
			//exit;
          //  $this->_getSession()->setCartWasUpdated(true);
	...
	public function addproductAction() {
			$use_moon = $this->getRequest()->getParam("use_moon");
			if(isset($use_moon)){
						$_cart_add_url = $this->getRequest()->getParam('cart_add_url');
						$cart_add_url = json_decode($_cart_add_url);
								$p=array();
								$p=explode("/",$cart_add_url);
						//$cart_add_url = $this->Moon_getCartURLParam($cart_add_url);	
						$oCart = $this->_getCart();
						$product = $this->Moon_initProduct($p[0]);
						$oCart->addproduct($product,1);
						$oCart->save();
						$this->_getSession()->setCartWasUpdated(true);
			}
			return;
    ...
    private function Moon_initProduct($product_id)
    {
